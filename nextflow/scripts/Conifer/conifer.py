...
def CF_analyze(args):
	...
	try: 
		...
		print (f'[INIT] Successfully read in {num_probes} probes from {probe_fn}')
	except IOError as e: 
		print (f'[ERROR] Cannot read probes file: {probe_fn}')
		sys.exit(0)
	...
	if len(rpkm_files) == 0:
		...
		print (f'[WARNING] Only found { len(rpkm_files)} samples... this is less than the recommended minimum, and CoNIFER may not analyze this dataset correctly!')
	elif len(rpkm_files) <= int(args.svd):
		print (f'[ERROR] The number of SVD values specified ({len(rpkm_files)}) must be less than the number of samples {int(args.svd)}. Either add more samples to the analysis or reduce the --svd parameter! Exiting.')
		sys.exit(0)
	else:
		print (f'[INIT] Found {len(rpkm_files)} RPKM files in {rpkm_dir}')
	
	# read in samples names and generate file list
	samples = {}
	for f in rpkm_files:
		s = '.'.join(f.split('/')[-1].split('.')[0:-1])
		print (f"[INIT] Mapping file to sampleID: {f} --> {s}")
		samples[s] = f
	...
	for i,s in enumerate(samples.keys()):
		...
		if len(t) != num_probes:
			...
			RPKM_data[:,i] = t
			print (f"[INIT] Successfully read RPKM data for sampleID: %{s}")
	
	RPKM_data = RPKM_data[:,0:len(samples)]
	print (f"[INIT] Finished reading RPKM files. Total number of samples in experiment: %{len(samples)} ({failed_samples} failed to read properly)")
	
	if len(samples) <= int(args.svd):
		print (f'[ERROR] The number of SVD values specified ({int(args.svd)}) must be less than the number of samples ({len(samples)}). Either add more samples to the analysis or reduce the --svd parameter! Exiting.')
		sys.exit(0)
